...
def load_documents(paths: Iterable[Path]) -> List[Document]:
    ...
    try:
        ...
        # Provide a clearer hint when PDF dependencies are missing.
        if "pypdf" in str(e).lower():
            msg = "PDF ingestion requires pypdf; install it via 'pip install pypdf'"
            log.error("PDF dependency missing", error=str(e))
            raise DocumentPortalException(msg, e) from e
        log.error("Import failure while loading documents", error=str(e))
